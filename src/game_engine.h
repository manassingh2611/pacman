...
T sens(const T& value, const T& lim) {
  if (value < lim && value > -lim) return 0;
  return value;
}
}  // namespace math
...
class CGameObject {
 public:
  ...
  T* findObjectByName(const std::string& name) {
    ...
    if (it != m_objects.end()) return dynamic_cast<T*>(*it);
    return nullptr;
  }
  template <typename T>
  T* findObjectByType() {
    for (auto& obj : m_objects)
      if (dynamic_cast<T*>(obj) != nullptr) return (T*)obj;
    return nullptr;
  }
  template <typename T>
  std::vector<T*> findObjectsByType() {
    ...
    for (auto& obj : m_objects) {
      if (dynamic_cast<T*>(obj) != nullptr) objects.push_back((T*)obj);
  ...
  void foreachObject(
      std::function<void(CGameObject*, bool& need_break)> predicate);
 ...
 protected:
  ...
  virtual void onPositionChanged(const Vector& new_pos,
...
class Timer : public CGameObject {
 public:
  ...
  void add(sf::Time time, T callable) {
    m_call_back_list.push_back(
        std::pair<sf::Time, std::function<void()>>(time, std::bind(callable)));
  }
  ~Timer();
...
template <typename T>
void ResourceManager<T>::loadFromFile(const std::string& name,
                                      const std::string& file_path) {
  assert(m_resources[name] == nullptr);  // allready exist
  m_resources[name] = new T();
  if (!m_resources[name]->loadFromFile(file_path))
    throw std::runtime_error(
        ("runtime error can't load resource: " + file_path).c_str());
}

template <>
inline void ResourceManager<sf::Music>::loadFromFile(
...
class CInputManager {
 private:
  std::unordered_map<sf::Keyboard::Key, bool> m_keys_prev, *m_keys_prev_ptr,
      m_keys_now, *m_keys_now_ptr;
  std::unordered_map<int, bool> m_jsk_btns_prev, *m_jsk_btns_prev_ptr,
      m_jsk_btns_now, *m_jsk_btns_now_ptr;
 ...
 public:
  ...
  void setupButton(const std::string& button,
...
class CSpriteSheet : public CGameObject {
 public:
  ...
  void load(const sf::Texture& texture, const Vector& off_set,
...
class Animator : public CGameObject {
 public:
  ...
  void create(const std::string& name, const sf::Texture& texture,
              const Vector& off_set, const Vector& size, int cols, int rows,
              float speed, AnimType anim_type = AnimType::forward_cycle);
  void create(const std::string& name, const sf::Texture& texture,
              const Rect& rect);
  void create(const std::string& name, const sf::Texture& texture,
              const std::vector<sf::IntRect>& rects, float speed);
  void play(const std::string& name);
  void update(int delta_time) override;
  void draw(sf::RenderWindow* wnd) override;
  ...
  void setSpriteOffset(const std::string& anim_name, int sprite_index,
                       const Vector& value);
  void setAnimOffset(float index);
...
class WaypointSystem : public CGameObject {
 ...
 public:
  ...
  void addPath(const std::vector<Vector>& path, float speed,
               bool align = false);
  bool isMoving() const;
  void stop();
  void update(int delta_time) override;
