...
Vector Vector::operator*(const float& k) const { return Vector(x * k, y * k); }
...
Vector::operator sf::Vector2f() const { return sf::Vector2f(x, y); }
...
Vector Vector::moveTowards(const Vector& current, const Vector& target,
                           float distance) {
...
Rect::Rect() { _left = _top = _width = _height = 0; }
...
Rect::Rect(const Vector& top_left, const Vector& size)
    : _left(top_left.x), _top(top_left.y), _width(size.x), _height(size.y) {}

float Rect::right() const { return _left + _width; }

float Rect::bottom() const { return _top + _height; }

Vector Rect::leftTop() const { return {_left, _top}; }

Vector Rect::rightTop() const { return {right(), _top}; }

Vector Rect::leftBottom() const { return {_left, bottom()}; }

Vector Rect::rightBottom() const { return {right(), bottom()}; }

Vector Rect::center() const { return {_left + _width / 2, _top + _height / 2}; }

Vector Rect::size() const { return {_width, _height}; }
...
bool Rect::isContain(const Vector& point) const {
  if (point.x >= _left && point.y >= _top && point.x < right() &&
      point.y < bottom())
    return true;
  return false;
}
...
bool Rect::isContainByX(const Vector& point) const {
  if (point.x >= _left && point.x < right()) return true;
  return false;
}

bool Rect::isContainByY(const Vector& point) const {
  if (point.y >= _top && point.y < bottom()) return true;
  return false;
}

bool Rect::isIntersect(const Rect& other) const {
  return abs(2 * (_left - other._left) + _width - other._width) <
             abs(_width + other._width) &&
         abs(2 * (_top - other._top) + _height - other._height) <
             abs(_height + other._height);
}
...
bool Rect::operator==(const Rect& other) const {
  if (_left == other._left && _top == other._top && _width == other._width &&
...
void Rect::setWidth(int width) { _width = width; }

void Rect::setHeight(int height) { _height = height; }
...
void Rect::setBottom(int bottom) { _height = bottom - _top; }

void Rect::setRight(int right) { _width = right - _left; }
