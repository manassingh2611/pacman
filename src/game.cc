...
void CPacManGameScene::resetScore() { m_score = 0; }
...
void CPacManGameScene::loadStage(const std::string& name) {
  m_walls->getMap()->loadFromFile({{'*', EMapBrickTypes::full},
                                  ...
                                  "assets/levels/" + name + ".txt");

  m_dots->fill(m_walls);

  m_fruit_cell = m_walls->getMap()->getCells(EMapBrickTypes::fruit)[0];
  m_fruit->setPosition(
      m_walls->toPixelCoordinates(m_fruit_cell + Vector(0.5, 0)));
  m_fruit->disable();
  m_fruit->hide();

  auto pills = findObjectsByType<CPill>();
  for (auto& pill : pills) removeObject(pill);
  ...
  m_pacman_spawn_position =
      m_walls->getMap()->getCells(EMapBrickTypes::pacman_spawn)[0] +
      Vector(0.5, 0);
  m_walls->lining();
}
...
void CPacManGameScene::initGhostsStates() {
  ...
  sf::Color ghost_colors[] = {sf::Color(255, 0, 0), sf::Color(255, 105, 180),
                              sf::Color(15, 255, 225), sf::Color(255, 140, 0)};

  CScatterState::Corner ghost_corners[] = {
      CScatterState::Corner::left_bottom, CScatterState::Corner::left_up,
      CScatterState::Corner::right_bottom, CScatterState::Corner::right_up};
  ...
  for (int i = 0; i < 4; ++i) {
    ...
    m_ghost_states[GhostStates::scatter][ghost_names[i]] =
        new CScatterState(ghost_corners[i]);
    m_ghost_states[GhostStates::frightened][ghost_names[i]] =
        CFrightenedState::getInstance();
    m_ghost_states[GhostStates::souls][ghost_names[i]] =
        new CSoulState(ghost_house_door_cell);
    m_ghost_states[GhostStates::borning][ghost_names[i]] =
        new CBorningState(ghost_house_door_cell);
    m_ghost_states[GhostStates::in_ghost_house][ghost_names[i]] =
        new CInHouseState(Vector(13.5, 12));
  }

  CGhostState* st[] = {new CBinkyState(), new CPinkyState(),
                       new CInkyState(m_ghosts[0]), new CClydeState()};
  for (int i = 0; i < 4; ++i) {
...
void CPacManGameScene::createGui() {
  ...
  m_big_text->setFontName(
      *CPacManGame::instance()->fontManager().get("main_font"));
  m_big_text->setFontStyle(sf::Text::Bold);
  m_big_text->setFontSize(30);
  m_big_text->setFontColor(sf::Color(50, 50, 50));
  m_big_text->setPosition(m_walls->size().x / 2 + 15,
                          m_walls->size().y / 2 + 42);
  addObject(m_big_text);

  m_flow_text =
      new CFlowText(*CPacManGame::instance()->fontManager().get("arial"));
  addObject(m_flow_text);

  m_score_label = new CLabel();
  m_score_label->setBounds(770, 40, 140, 30);
  m_score_label->setFontName(
      *CPacManGame::instance()->fontManager().get("score_font"));
...
void CPacManGameScene::goToMainMenu() {
  CPacManGameScene* game_scene =
      CPacManGame::instance()
          ->getRootObject()
          ->findObjectByName<CPacManGameScene>("game_scene");
  CMainMenuScene* menu_scene =
      CPacManGame::instance()
          ->getRootObject()
          ->findObjectByName<CMainMenuScene>("menu_scene");
...
void CPacManGameScene::update(int delta_time) {
  ...
  if (!isEnabled() || !m_pacman->isEnabled()) return;
  ...
  if (m_dots->eat(player_claster.x, player_claster.y)) {
    ...
    if (m_dots->amount() == 0) {
      ...
      m_wave_timer->clear();
      m_wave_timer->add(sf::seconds(3),
                        std::bind(&CPacManGameScene::goToMainMenu, this));
      return;
    }
    if (m_dots->amount() == m_dots->maxDots() - 70 ||
        m_dots->amount() == m_dots->maxDots() - 170) {
      m_fruit->enable();
      m_fruit->show();
      m_fruit_timer->clear();
      m_fruit->setFlashed(true);
      m_fruit_timer->add(sf::seconds(3),
                         [this]() { m_fruit->setFlashed(false); });
      m_fruit_timer->add(sf::seconds(20), [this]() {
        m_fruit->disable();
        m_fruit->hide();
      });
      m_fruit_timer->add(sf::seconds(17),
                         [this]() { m_fruit->setFlashed(true); });
    }
  ...
  if (m_walls->toMapCoordinates(m_fruit->getPosition()) == player_claster &&
      m_fruit->isEnabled()) {
      ...
      switch (monster->currentStateType()) {
        ...
        default: {
          ...
          if (m_lives > 0) {
            ...
            enableActors(false);
            m_wave_timer->clear();
            m_wave_timer->add(sf::seconds(3), [this]() {
              spawnGhosts();
              spawnPacman();
            });
            return;
  ...
  for (auto& obj : m_ghosts) {
    if (obj->currentStateType() == CGhostState::Soul && !obj->isMoving()) {
      obj->setState(
          m_ghost_states[GhostStates::in_ghost_house][obj->getName()]);
      m_born_timer->add(sf::seconds(5), [this, obj]() {
        setGhostState(obj, GhostStates::borning);
        CPacManGame::instance()->playSound("ghost_regenerate");
      });
    }
  ...
  foreachObject([&pill, this](CGameObject* object) {
    if (object->isEnabled() && object->getName() == "Pill" &&
        m_walls->toMapCoordinates(m_pacman->getPosition()) ==
            m_walls->toMapCoordinates(object->getPosition())) {
      pill = object;
      m_wave_timer->disable();
      setGhostsToFrightenedState();
      m_pill_timer->clear();
      m_pill_timer->add(sf::seconds(7),
                        [this]() { CFrightenedState::setFlashed(true); });
      m_pill_timer->add(sf::seconds(10), [this]() {
        m_wave_timer->enable();
...
void CPacManGameScene::spawnGhosts() {
  ...
  int t = 0;
  m_wave_timer->clear();
  m_wave_timer->add(sf::seconds(t += 2), [this]() {
    m_big_text->setString("");
    enableActors(true);
  });
  ...
  m_wave_timer->add(sf::seconds(t += 20),
                    [this]() { setGhostsToScatterState(); });
  m_wave_timer->add(sf::seconds(t += 7), [this]() { setGhostsToChaseState(); });
  m_wave_timer->add(sf::seconds(t += 20),
                    [this]() { setGhostsToScatterState(); });
...
void CPacManGameScene::setGhostsToScatterState() {
  ...
  for (auto& obj : m_ghosts)
    if (isChaseState(obj->currentStateType()) ||
        obj->currentStateType() == CGhostState::Type::Frightened)
      setGhostState(obj, GhostStates::scatter);
}
...
CMainMenuScene::CMainMenuScene() {
  ...
  m_logo = new CLabel();
  m_logo->setSprite(
      sf::Sprite(*CPacManGame::instance()->textureManager().get("texture"),
                 sf::IntRect(5, 148, 240, 50)));
  ...
  m_ghost_name->setFontName(
      *CPacManGame::instance()->fontManager().get("menu_font"));
  ...
  m_buttons[0]->onClick([timer, root]() {
    root->findObjectByName("game_scene")->turnOn();
...
void CMainMenuScene::reset() {
  ...
  for (int i = 0; i < ghosts_amount; ++i)
    timer->add(sf::seconds(6 + i), [this, i]() {
      m_ghosts[i]->setState(m_ghost_states[(int)CToyState::State::Soul]);
    });
  ...
  for (int i = 0; i < ghosts_amount; ++i) {
    float dt = i * 7;
    timer->add(sf::seconds(12.5 + dt), [this, i] {
      m_ghosts[i]->setSpeed(0.4);
      m_ghosts[i]->setMovingPath({{-50, 300}, {650, 300}});
    });
    timer->add(sf::seconds(14.3 + dt), [this, i] {
      m_ghost_name->setString(m_ghosts[i]->getName());
      m_ghost_name->setFontColor(m_ghosts[i]->color());
    });
    timer->add(sf::seconds(16.5 + dt), [this, i] {
      m_ghost_name->setString("");
      m_ghosts[i]->setMovingPath({{650, 300}, {1100, 300}});
    });
  }
...
void CMainMenuScene::events(const sf::Event& event) {
  ...
  if (event.type == sf::Event::KeyPressed) {
    ...
    if (event.joystickButton.button == 7) action = start;
  }
...
CPacManGame* CPacManGame::instance() {
  if (s_instance == NULL) s_instance = new CPacManGame();
  return s_instance;
}

CPacManGame::CPacManGame() : CGame("PacMan", {1000, 850}) {
  // Load resources
  textureManager().loadFromFile("texture", "assets/sprites.png");
  textureManager().get("texture")->setSmooth(true);

  for (auto& font_name : {"arial", "menu_font", "main_font", "score_font"})
    fontManager().loadFromFile(
        font_name, "assets/fonts/" + std::string(font_name) + ".ttf");

  for (auto& sound_name : {"begininng", "eat_dot", "ghosts_frightened",
                           "ghost_eaten", "life_lost", "ghost_regenerate"})
    soundManager().loadFromFile(
        sound_name, "assets/sounds/" + std::string(sound_name) + ".wav");
...
bool CButton::hasFocus() const { return m_focus; }

void CButton::draw(sf::RenderWindow* window) { CLabel::draw(window); }

void CButton::onClick(const std::function<void()>& call_back) {
  m_call_back = call_back;
}

void CButton::onMouseEnter() { setFillColor(sf::Color(200, 200, 255)); }

void CButton::onMouseLeave() { setFillColor(sf::Color::White); }
...
CLifeBar::CLifeBar(const Vector& pos) {
  setPosition(pos);
  sf::Texture* texture =
      CPacManGame::instance()->textureManager().get("texture");
...
void CLifeBar::setValue(int value) { m_value = value; }
...
void CPacman::init() {
  ...
  sf::Texture* texture =
      CPacManGame::instance()->textureManager().get("texture");
  m_animator.create("right", *texture, {0, 32}, {48, 48}, 4, 1, 0.03,
                    AnimType::forward_backward_cycle);
  m_animator.create("left", *texture, {48, 32}, {-48, 48}, 4, 1, 0.03,
                    AnimType::forward_backward_cycle);
  m_animator.create("down", *texture, {0, 32}, {48, 48}, 4, 1, 0.03,
                    AnimType::forward_backward_cycle);
  m_animator.create("up", *texture, {0, 32}, {48, 48}, 4, 1, 0.03,
                    AnimType::forward_backward_cycle);
...
void CPacman::update(int delta_time) {
  ...
  if (delta_time == 0 || !m_walls) return;
  ...
  bool can_turn = !(!m_walls->inBounds(next_cell) ||
                    m_walls->getMapCell(next_cell) != EMapBrickTypes::empty);

  if (input_direction != Vector::zero && input_direction != direction &&
      can_turn) {
    Vector player_cell = m_walls->toMapCoordinates(getPosition());
    Vector finish_cell = m_walls->getMap()->traceLine(
        player_cell, input_direction, EMapBrickTypes::empty);
    m_waypoint_system->addPath(
        m_walls->toPixelCoordinates({player_cell, finish_cell}), NORMAL_SPEED);
  }
  ...
  if (direction == Vector::left) {
    if (getPosition().x < 5) {
      setPosition(m_walls->toPixelCoordinates(
          {m_walls->getMap()->width() - 1.f, player_pos.y}));
      Vector player_cell = m_walls->toMapCoordinates(getPosition());
      Vector finish_cell = m_walls->getMap()->traceLine(
          player_cell, Vector::left, EMapBrickTypes::empty);
      m_waypoint_system->addPath(
          m_walls->toPixelCoordinates({player_cell, finish_cell}),
          NORMAL_SPEED);
    }
    ...
    if (getPosition().x > (m_walls->size().x - 30)) {
      ...
      Vector player_cell = m_walls->toMapCoordinates(getPosition());
      Vector finish_cell = m_walls->getMap()->traceLine(
          player_cell, Vector::right, EMapBrickTypes::empty);
      m_waypoint_system->addPath(
          m_walls->toPixelCoordinates({player_cell, finish_cell}),
          NORMAL_SPEED);
    }
...
CGhost::CGhost(const std::string& name, CGameObject* target, CWalls* walls) {
  ...
  for (int i = 6; i < 10; ++i) m_sprite_sheet[i].setOrigin(-4, -34);

  m_waypoint_system = new WaypointSystem();
  addObject(m_waypoint_system);
}

CGhostState::Type CGhost::currentStateType() { return m_ghost_state->type(); }

void CGhost::update(int delta_time) {
  CGameObject::update(delta_time);
  m_time += delta_time;
  if (m_ghost_state) m_ghost_state->update({delta_time, this, m_walls});
};
...
void CGhost::drawEyes(sf::RenderWindow* window) {
  ...
  if (getDirection() == Vector::zero) window->draw(m_sprite_sheet[1]);
}
...
void CGhost::setTarget(CGameObject* target) { m_target = target; }

CGameObject* CGhost::target() { return m_target; }

void CGhost::setColor(sf::Color color) {
  m_sprite_sheet[0].setColor(color);
  for (int i = 6; i < 10; ++i) m_sprite_sheet[i].setColor(color);
}
...
void CGhost::setState(CGhostState* state) {
  if (m_ghost_state != NULL) m_ghost_state->deactivate({0, this, m_walls});
  m_ghost_state = state;
  m_ghost_state->activate({0, this, m_walls});
}

Vector CGhost::getTargetPos() const { return m_target_pos; }

bool CGhost::isMoving() const { return m_waypoint_system->isMoving(); }
...
void CGhost::moveToTarget(const Vector& target_pos) {
  ...
  std::vector<Vector> nodes =
      m_walls->getMap()->getNeighborNodes(object_cell, EMapBrickTypes::empty);
  ...
  m_waypoint_system->addPath(
      m_walls->toPixelCoordinates({object_cell, next_node}), getSpeed());
}
...
void CGhost::stop() { m_waypoint_system->stop(); }

void CGhost::setSpeed(float speed) { m_speed = speed; }

float CGhost::getSpeed() const { return m_speed; }

//-------------------------------------------------------------------------------------------------

CGhostState::CGhostState(Type type) : m_type(type) {}

CGhostState::Type CGhostState::type() const { return m_type; }

void CGhostState::draw(const CGhostStateContex& contex,
...
CWalls::CWalls(int width, int height) {
  ...
  m_sprite_sheet.load(*texture, {{0, 0, 32, 32},
                                 {32, 0, 32, 32},
                                 {64, 0, 32, 32},
                                 {96, 0, -32, 32},
                                 {64, 0, 32, 32},
...
void CWalls::draw(sf::RenderWindow* window) {
  ...
  sf::RectangleShape retangle(sf::Vector2f(m_map->width() * CLASTER_SIZE,
                                           m_map->height() * CLASTER_SIZE));
...
void CWalls::lining() {
  ...
  for (int x = 0; x < map->width(); ++x)
    for (int y = 0; y < map->height(); ++y) {
      if (map->getCell(x, y) >= EMapBrickTypes::in_corn_left_down) continue;

      if (x < map->width() - 1 && map->getCell(x, y) != EMapBrickTypes::empty &&
          map->getCell(x + 1, y) == EMapBrickTypes::empty)
        map->setCell(x, y, EMapBrickTypes::left);

      else if (x > 0 && map->getCell(x, y) != EMapBrickTypes::empty &&
               map->getCell(x - 1, y) == EMapBrickTypes::empty)
      ...
      else if (y > 0 && map->getCell(x, y) != EMapBrickTypes::empty &&
               map->getCell(x, y - 1) == EMapBrickTypes::empty)
...
CWalls::~CWalls() { delete m_map; }

Vector CWalls::toMapCoordinates(const Vector& global_pos) {
  return {(int)round(global_pos.x / CLASTER_SIZE),
          (int)round(global_pos.y / CLASTER_SIZE)};
}
...
std::vector<Vector> CWalls::toPixelCoordinates(std::vector<Vector>&& path) {
  for (auto& v : path) v *= CLASTER_SIZE;
  return path;
}
...
Vector CWalls::alignToMap(const Vector& position) const {
  return {(int)CLASTER_SIZE * round(position.x / CLASTER_SIZE),
          CLASTER_SIZE * round(position.y / CLASTER_SIZE)};
}
...
TileMap<EMapBrickTypes>* CWalls::getMap() { return m_map; }

bool CWalls::inBounds(const Vector& vec) const { return m_map->inBounds(vec); }
  ...
  for (int x = lt.x; x < rb.x; ++x)
    for (int y = lt.y; y < rb.y; ++y)
      if (m_map->getCell(x, y) != allowed_cell_type)
        if (Rect(x * CLASTER_SIZE, y * CLASTER_SIZE, CLASTER_SIZE, CLASTER_SIZE)
                .isIntersect(rect))
          return true;
  return false;
}
...
CDots::CDots(CWalls* walls) {
  m_walls = walls;
  m_dots_map =
      new TileMap<bool>(walls->getMap()->width(), walls->getMap()->height());
  m_saved_map =
      new TileMap<bool>(walls->getMap()->width(), walls->getMap()->height());
  ...
  for (int x = 0; x < m_dots_map->width(); ++x)
    for (int y = 0; y < m_dots_map->height(); ++y)
      if (m_dots_map->getCell(x, y) == true) {
        m_shape.setPosition(
            m_walls->toPixelCoordinates(Vector(x + 0.5f, y + 0.5f)));
        window->draw(m_shape);
      }
}
...
int CDots::maxDots() const { return m_max_dots; }
...
int CDots::amount() const { return m_dots_counter; }
