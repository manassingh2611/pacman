...
float toFloat(const std::string& str) { return std::stof(str); }
...
void drawLinearSprite_v(sf::Sprite sprite, const sf::Rect<int>& draw_area,
                        sf::RenderWindow* render_window) {
  if (!draw_area.height) return;
...
void drawLinearSprite_h(sf::Sprite sprite, const sf::Rect<int>& draw_area,
                        sf::RenderWindow* render_window) {
  if (!draw_area.width) return;
...
Property::Property() { m_type = Type::NoInit; }

Property::~Property() {
  if (m_type == Type::String) delete string_data;
}
...
bool Property::isValid() const { return m_type != Type::NoInit; }

//----------------------------------------------------------------------------------------------

void CEventManager::pushEvent(const sf::Event& event) {
  for (auto& sub : m_subcribes) sub->events(event);
}
...
void CEventManager::unsubcribe(CGameObject* object) {
  ...
  if (it != m_subcribes.end()) m_subcribes.erase(it);
}
...
CInputManager::CInputManager() {
  ...
  for (auto& ax : m_axis_keys) ax = sf::Keyboard::Key::Unknown;
}
...
bool CInputManager::isKeyJustPressed(const sf::Keyboard::Key& key) const {
  if (m_keys_prev_ptr->count(key))
    if (!(*m_keys_prev_ptr)[key] && (*m_keys_now_ptr)[key]) return true;
  return false;
}
...
bool CInputManager::isKeyPressed(const sf::Keyboard::Key& key) const {
  if (m_keys_now_ptr->count(key)) return (*m_keys_now_ptr)[key];
  return false;
}
...
Vector CInputManager::getXYAxis() const {
  ...
  if (sf::Joystick::isConnected(0)) {
    value.x =
        sf::Joystick::getAxisPosition(0, sf::Joystick::Axis::PovX) / 100.f;
    value.y =
        sf::Joystick::getAxisPosition(0, sf::Joystick::Axis::PovY) / 100.f;
    value.x = math::sens(value.x, 0.5f);
    value.y = -math::sens(value.y, 0.5f);
  }

  if (m_axis_keys[0] != -1 && sf::Keyboard::isKeyPressed(m_axis_keys[0]))
    value.y = -1;
  if (m_axis_keys[1] != -1 && sf::Keyboard::isKeyPressed(m_axis_keys[1]))
    value.x = 1;
  if (m_axis_keys[2] != -1 && sf::Keyboard::isKeyPressed(m_axis_keys[2]))
    value.y = 1;
  if (m_axis_keys[3] != -1 && sf::Keyboard::isKeyPressed(m_axis_keys[3]))
    value.x = -1;
...
bool CInputManager::isButtonPressed(const std::string& button) const {
  if (m_btn_to_key.count(button) && isKeyPressed(m_btn_to_key.at(button)))
    return true;
  if (m_jsk_btn_to_key.count(button) &&
      isJoystickButtonPressed(m_jsk_btn_to_key.at(button)))
    return true;
  return false;
}
...
bool CInputManager::isButtonDown(const std::string& button) const {
  if (m_btn_to_key.count(button) && isKeyJustPressed(m_btn_to_key.at(button)))
    return true;
  if (m_jsk_btn_to_key.count(button) &&
      isJoystickButtonJustPressed(m_jsk_btn_to_key.at(button)))
    return true;
  return false;
}
...
sf::Keyboard::Key CInputManager::toKey(const std::string& str) {
  ...
  static const std::map<std::string, sf::Keyboard::Key> key_map = {
      ...
      {"Space", sf::Keyboard::Space}, {"LShift", sf::Keyboard::LShift},
      {"Enter", sf::Keyboard::Enter}, {"Return", sf::Keyboard::Return},
  };

  if (key_map.find(str) != key_map.end()) return key_map.at(str);
}

void CInputManager::setupButton(const std::string& button,
...
bool CInputManager::isJoystickButtonPressed(int index) const {
  if (m_jsk_btns_now_ptr->count(index)) return (*m_jsk_btns_now_ptr)[index];
  return false;
}
...
const Vector& CGameObject::getPosition() const { return m_pos; }
...
void CGameObject::setPosition(float x, float y) { setPosition(Vector(x, y)); }

void CGameObject::move(const Vector& point) { m_pos += point; }

void CGameObject::setDirection(const Vector& vec) { m_direction = vec; }

Vector CGameObject::getDirection() const { return m_direction; }

void CGameObject::setSize(const Vector& size) { m_size = size; }

Rect CGameObject::getBounds() const { return Rect(m_pos, m_size); }
...
CGameObject* CGameObject::getParent() const { return m_parent; }
...
void CGameObject::setName(const std::string& name) { m_name = name; }

const std::string& CGameObject::getName() const { return m_name; }

void CGameObject::setProperty(const std::string& name,
...
void CGameObject::disable() { m_enable = false; }

void CGameObject::enable() { m_enable = true; }

bool CGameObject::isEnabled() const { return m_enable; }

void CGameObject::hide() { m_visible = false; }

void CGameObject::show() { m_visible = true; }

bool CGameObject::isVisible() const { return m_visible; }
...
CGameObject* CGameObject::findObjectByName(const std::string& name) {
  auto it = std::find_if(m_objects.begin(), m_objects.end(),
                         [this, &name](const CGameObject* obj) -> bool {
...
void CGameObject::draw(sf::RenderWindow* window) {
  if (isVisible())
    for (auto& obj : m_objects)
      if (obj->isVisible()) obj->draw(window);
}

void CGameObject::postDraw(sf::RenderWindow* window) {
  if (isVisible())
    for (auto& obj : m_objects)
      if (obj->isVisible()) obj->postDraw(window);
}

void CGameObject::foreachObject(std::function<void(CGameObject*)> predicate) {
  for (auto& obj : m_objects) predicate(obj);
}
void CGameObject::foreachObject(
    std::function<void(CGameObject*, bool&)> predicate) {
...
std::vector<std::function<void()>> CGameObject::m_preupdate_actions =
    std::vector<std::function<void()>>();

void CGameObject::invokePreupdateActions() {
  for (auto& action : m_preupdate_actions) action();
  m_preupdate_actions.clear();
}

//---------------------------------------------------------------------------------------------------------

Timer::Timer() { setName("Timer"); }
...
void Timer::update(int miliseconds) {
  ...
  for (auto it = m_call_back_list.begin(); it != m_call_back_list.end();) {
    ...
    if (pair.first <= sf::Time() || !pair.second) {
      if (pair.second) pair.second.operator()();
...
void CGame::updateStats(const sf::Time time) {
  if (time > m_max_time) m_max_time = time;
  if (time < m_min_time) m_min_time = time;
...
void CGame::setClearColor(const sf::Color& color) { m_clear_color = color; }

void CGame::run() {
  m_window =
      new sf::RenderWindow(sf::VideoMode(m_screen_size.x, m_screen_size.y),
                           m_root_object->getName());
  init();
...
CGameObject* CGame::getRootObject() { return m_root_object; }
...
CTextureManager& CGame::textureManager() { return m_texture_manager; }

CFontManager& CGame::fontManager() { return m_font_manager; }

CSoundManager& CGame::soundManager() { return m_sound_manager; }

CEventManager& CGame::eventManager() { return m_event_manager; }

CInputManager& CGame::inputManager() { return m_input_manager; }

CMusicManager& CGame::musicManager() { return m_music_manager; }

void CGame::playMusic(const std::string& name) { m_music_manager.play(name); }

void CGame::stopMusic() { m_music_manager.stop(); }

void CGame::playSound(const std::string& name) {
  const int SOUND_BUFFER_SIZE = 40;
  int i = 0;
  while (m_sounds_buf[i].getStatus() == sf::Sound::Playing) {
    if (i >= SOUND_BUFFER_SIZE) break;
    ++i;
  }

  if (i >= SOUND_BUFFER_SIZE) throw std::runtime_error("sound buffer overflow");
...
void CSpriteSheet::load(const sf::Texture& texture,
                        const std::vector<sf::IntRect>& rects) {
  m_sprites.clear();
  for (auto& rect : rects) m_sprites.emplace_back(texture, rect);
  setSpriteIndex(0);
}

void CSpriteSheet::load(const sf::Texture& texture, const Vector& off_set,
                        const Vector& size, int cols, int rows) {
  m_sprites.clear();
  for (int y = 0; y < rows; ++y)
    for (int x = 0; x < cols; ++x)
      m_sprites.emplace_back(
          texture,
          sf::IntRect(x * std::abs(size.x) + off_set.x,
                      y * std::abs(size.y) + off_set.y, size.x, size.y));
  setSpriteIndex(0);
  setAnimType(AnimType::forward);
}

void CSpriteSheet::reset() { m_index = 0; }

bool CSpriteSheet::empty() const { return m_sprites.empty(); }

void CSpriteSheet::setSpriteIndex(int index) {
  assert(index >= 0 && index < m_sprites.size());
  m_current_sprite = &m_sprites[index];
}
sf::Sprite* CSpriteSheet::currentSprite() { return m_current_sprite; }
...
void CSpriteSheet::draw(sf::RenderWindow* wnd) {
  switch (m_anim_type) {
    ...
    case (AnimType::forward_stop): {
      ...
      if (current_slide < m_sprites.size()) setSpriteIndex(current_slide);
      break;
    }
...
void CSpriteSheet::setPosition(sf::Vector2f pos) {
  m_position = pos;
  for (auto& sprite : m_sprites) sprite.setPosition(m_position + m_torigin);
}

void CSpriteSheet::setOrigin(const Vector& pos) {
  for (auto& sprite : m_sprites) sprite.setOrigin(pos);
}
...
sf::Vector2f CSpriteSheet::getPosition() const { return m_position; }

void CSpriteSheet::scale(float fX, float fY) {
  for (auto& sprite : m_sprites) sprite.scale(fX, fY);
}

void CSpriteSheet::update(int delta_time) {
  if (isEnabled() && m_speed) m_index += m_speed * delta_time;
}

void CSpriteSheet::setSpeed(float speed) { m_speed = speed; }

void CSpriteSheet::setColor(const sf::Color& color) {
  for (auto& sprite : m_sprites) sprite.setColor(color);
}

void CSpriteSheet::setRotation(float angle) {
  if (angle == m_sprites[0].getRotation()) return;
...
void CSpriteSheet::invert_h() {
  ...
  // sf::IntRect new_rect(rect.width + rect.left, rect.top, -rect.width,
  // rect.height); return sf::Sprite(*texture, new_rect);
}
...
void CSpriteSheet::setAnimOffset(float i) { m_index += i; }

AnimType CSpriteSheet::animType() const { return m_anim_type; }
//------------------------------------------------------------------------------------------------------------------------------------------------------
...
void Pallete::create(const std::initializer_list<sf::Color>& original_colors,
  ...
  for (auto original_color : original_colors)
    colors1[i++] =
        sf::Glsl::Vec3(original_color.r / 255.f, original_color.g / 255.f,
                       original_color.b / 255.f);
  i = 0;
  for (auto swaped_color : swaped_colors)
    colors2[i++] = sf::Glsl::Vec3(
        swaped_color.r / 255.f, swaped_color.g / 255.f, swaped_color.b / 255.f);
...
void Pallete::apply() { sf::Shader::bind(&m_shader); }

void Pallete::cancel() { sf::Shader::bind(NULL); }
...
void Animator::create(const std::string& name, const sf::Texture& texture,
                      const Vector& off_set, const Vector& size, int cols,
                      int rows, float speed, AnimType anim_type) {
...
void Animator::create(const std::string& name, const sf::Texture& texture,
                      const Rect& rect) {
  CSpriteSheet* animation = new CSpriteSheet();
  animation->load(texture, {{(int)rect.left(), (int)rect.top(),
...
void Animator::create(const std::string& name, const sf::Texture& texture,
                      const std::vector<sf::IntRect>& rects, float _speed) {
  CSpriteSheet* animation = new CSpriteSheet();
...
void Animator::update(int delta_time) {
  if (isEnabled()) m_current_animation->update(delta_time);
}
void Animator::draw(sf::RenderWindow* wnd) {
  if (isVisible()) {
    if (m_pallete) m_pallete->apply();

    m_current_animation->setPosition(getPosition());
    m_current_animation->draw(wnd);

    if (m_pallete) m_pallete->cancel();
  }
}

void Animator::setPallete(Pallete* pallete) { m_pallete = pallete; }

void Animator::flipX(bool value) {
  if (value != m_flipped) {
    m_flipped = value;
    for (auto& animation : m_animations) animation.second->flipX(value);
  }
}

void Animator::setColor(const sf::Color& color) {
  for (auto& animation : m_animations) animation.second->setColor(color);
}

void Animator::setAnimOffset(float index) {
  for (auto& animation : m_animations) animation.second->setAnimOffset(index);
}
...
void Animator::setSpriteOffset(const std::string& anim_name, int sprite_index,
                               const Vector& value) {
...
void Animator::scale(float fX, float fY) {
  for (auto& animation : m_animations) animation.second->scale(fX, fY);
}
...
void CMusicManager::play(const std::string& name) {
  if (name != "") m_current_music = name;
  assert(m_resources[m_current_music]);  // unknown resource
  if (!m_current_music.empty()) m_resources[m_current_music]->play();
}
...
void CMusicManager::pause() {
  for (auto music : m_resources) music.second->pause();
}

void CMusicManager::setPitch(float value) {
  for (auto music : m_resources) music.second->setPitch(value);
}
...
bool CFlowText::isFlashing() const { return m_flashing; }
...
void CFlowText::update(int delta_time) {
  if (m_flashing) {
    ...
    if (m_color >= 255) m_flashing = false;
...
void CFlowText::setTextSize(int size) { m_text.setCharacterSize(size); }
...
void CLabel::setBounds(int x, int y, int w, int h) {
  ...
  if (m_sprite.getTexture()) {
    ...
    m_sprite.setScale((float)w / m_sprite.getTextureRect().width,
                      (float)h / m_sprite.getTextureRect().height);
  }
}

void CLabel::setSprite(const sf::Sprite& sprite) { m_sprite = sprite; }

void CLabel::setFontColor(const sf::Color& color) {
  m_text.setFillColor(color);
}

void CLabel::setFontSize(int size) { m_text.setCharacterSize(size); }

void CLabel::setFontName(const sf::Font& font) { m_text.setFont(font); }

void CLabel::setFontStyle(sf::Uint32 style) { m_text.setStyle(style); }

void CLabel::setTextAlign(int value) { m_text_align = value; }

void CLabel::setString(const std::string& str) { m_text.setString(str); }
  ...
  if (!m_text.getString().isEmpty()) {
    if (m_text_align == center)
      m_text.setPosition(getPosition() + m_rect.size() / 2 -
                         Vector(m_text.getGlobalBounds().width,
...
void CLabel::onPropertySet(const std::string& name) {
  ...
  if (name == "text") setString(getProperty("text").asString());
}

void CLabel::onActivated() {
  if (getProperty("x").isValid()) {
    setBounds(getProperty("x").asFloat(), getProperty("y").asFloat(),
              getProperty("width").asFloat(), getProperty("height").asFloat());
    setString(getProperty("text").asString());
  }
  if (getProperty("hided").isValid() && getProperty("hided").asBool()) hide();
}

Rect CLabel::getBounds() const { return m_rect; }

sf::Sprite& CLabel::getSprite() { return m_sprite; }
...
CLabel* CLabel::clone() const {
  ...
  new_label->setBounds(bounds.left(), bounds.top(), bounds.width(),
                       bounds.height());
...
WaypointSystem::WaypointSystem() { setName("WaypointSystem"); }

CGameObject* WaypointSystem::getObject() { return getParent(); }

void WaypointSystem::addPath(const std::vector<Vector>& path, float speed,
                             bool align) {
  m_path.clear();
  m_path = path;
  m_speed = speed;
  m_length = 0;
  if (align) m_path.insert(m_path.begin(), getObject()->getPosition());
}

bool WaypointSystem::isMoving() const { return !m_path.empty(); }
...
Vector collsionResponse(const Rect& own_rect, const Vector& own_speed,
  ...
  if (!intersection.width() || !intersection.height()) return new_pos;
  ...
  if (dt > delta_time) dt = delta_time;
